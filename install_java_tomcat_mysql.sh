...
echo "--- MySQL Server 설치 시작 ---"
apt install -y mysql-server
echo "MySQL 서비스 시작 및 활성화..."
systemctl start mysql
systemctl enable mysql
echo "MySQL 보안 설정 시작 (root 비밀번호 설정, 익명 사용자 제거 등)"
echo "이 단계에서 MySQL 보안 설정을 수동으로 진행해야 합니다."
echo "엔터 키를 누른 후 'mysql_secure_installation' 프롬프트에 따라 진행해주세요."
read -p "진행하시려면 엔터 키를 누르세요..."
mysql_secure_installation
echo "--- MySQL Server 설치 완료 ---"
